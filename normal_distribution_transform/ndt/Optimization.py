...
class Optimization:
    ...
    def newtons_method( self, target_pc: TargetPointCloud, initial_se3: np.ndarray, epsilon: float = 0.00001, max_iterations: int = 100 ):
    ...
    def levenberg_marquardt( self, target_pc: TargetPointCloud, initial_se3: np.ndarray, epsilon: float = 0.00001, max_iterations: int = 10 ):
    ...
    def f( self, x: list[Point] ) -> float:
        ...
        for x_i in x:
            ...
            if( type( v ) == Voxel ):
                s += v.get_score( x_i.pos, self.d1, self.d2 )
