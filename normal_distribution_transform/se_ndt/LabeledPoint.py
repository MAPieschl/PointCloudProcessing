...
class LabeledPoint:
    def __init__( self, pos: np.ndarray, label: str, v: SemanticVoxel | None ):
    ...
    def is_contained_by( self ) -> SemanticVoxel | None:
        return self.voxel
    
    def move_relative( self, delta_vec6: np.ndarray, v: Callable[[np.ndarray], SemanticVoxel | None] ) -> None:
    ...
    def set_position( self, vec6: np.ndarray, v: Callable[[np.ndarray], SemanticVoxel | None] ) -> None:
